...
#define paths_equal(p1, p2)                                                    \
  do {                                                                         \
    ...
    if (__r = makedirs(p1),                                                    \
        !test_assert(0 == __r, "makedirs(\"%s\"): %s", p1, test_errstr(__r)))  \
      break;                                                                   \
    if (__r = stat(p1, &__s1),                                                 \
        !test_assert(0 == __r, "stat(\"%s\"): %s", p1, test_errstr(errno)))    \
      break;                                                                   \
    if (__r = stat(p2, &__s2),                                                 \
        !test_assert(0 == __r, "stat(\"%s\"): %s", p2, test_errstr(errno)))    \
      break;                                                                   \
...
static void vfstest_s5fs_vm(void) {
  ...
  test_assert(MAP_FAILED !=
                  (addr = mmap(0, strlen(TESTSTR), PROT_READ | PROT_WRITE,
                               MAP_PRIVATE, fd, 0)),
              NULL);
  ...
  test_assert(MAP_FAILED != mmap(addr, strlen(TESTSTR), PROT_READ | PROT_WRITE,
  ...
  test_assert(MAP_FAILED ==
                  mmap(0, 1024, PROT_READ, MAP_FIXED | MAP_PRIVATE, fd, 0),
              NULL);
  test_assert(MAP_FAILED == mmap(0, 1024, PROT_READ, MAP_PRIVATE, fd, 0x12345),
              NULL);
  ...
  test_assert(MAP_FAILED ==
                  mmap(0, 1024, PROT_READ, MAP_PRIVATE | MAP_FIXED, fd, 0),
              NULL);
  syscall_success(close(fd));
