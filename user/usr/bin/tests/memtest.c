...
#define syscall_success(expr)                                                  \
  test_assert(0 <= (expr), "\nunexpected error: %s (%d)", test_errstr(errno),  \
              errno)
...
static int test_mmap_bounds(void) {
  ...
  test_assert(MAP_FAILED !=
                  (addr = mmap(NULL, PAGE_SIZE * 3, PROT_READ | PROT_WRITE,
                               MAP_PRIVATE, fd, 0)),
              NULL);
  ...
  /* Remap as read-only */
  test_assert(addr == mmap(addr, 1, PROT_READ, MAP_PRIVATE | MAP_FIXED, fd, 0),
              NULL);
  ...
  test_assert(
      addr == mmap(addr, PAGE_SIZE, PROT_EXEC, MAP_PRIVATE | MAP_FIXED, fd, 0),
      NULL);
  assert_fault(char foo = *(char *)addr, "");
  assert_fault(char foo = *((char *)addr + PAGE_SIZE - 1), "");
  assert_fault(*((char *)addr) = 'a', "");

  test_assert(addr == mmap(addr, PAGE_SIZE, 0, MAP_PRIVATE | MAP_FIXED, fd, 0),
              NULL);
  assert_fault(char foo = *(char *)addr, "");
  assert_fault(char foo = *((char *)addr + PAGE_SIZE - 1), "");
  assert_fault(*((char *)addr) = 'a', "");
...
static int test_munmap(void) {
  ...
  test_assert(MAP_FAILED !=
                  (addr = mmap(NULL, PAGE_SIZE * 20, PROT_READ | PROT_WRITE,
  ...
  *(addr + PAGE_SIZE * 12) = '$';
  ...
  test_assert(MAP_FAILED !=
                  (middle = mmap(addr + PAGE_SIZE * 10, PAGE_SIZE,
...
static int test_mmap_fill(void) {
  ...
  test_assert(MAP_FAILED !=
                  (hi = mmap(NULL, 1, 0, MAP_ANON | MAP_PRIVATE, -1, 0)),
              NULL);
...
#define MID_ADDR ((char *)0x80000000)
  ...
  if (MID_ADDR < hi) {
    test_assert(lo == mmap(NULL, (size_t)((uintptr_t)MID_ADDR - (uintptr_t)lo),
                           0, MAP_ANON | MAP_PRIVATE, -1, 0),
                NULL);
  }
  ...
  test_assert(MAP_FAILED != mmap(NULL, (size_t)((uintptr_t)addr -
                                                (uintptr_t)USER_MEM_LOW),
                                 0, MAP_ANON | MAP_PRIVATE, -1, 0),
              NULL);
