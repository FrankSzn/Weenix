...
static module_t *_ldlinkobj(module_t *info, char *baseaddr, Elf32_Dyn *dyn) {
  ...
  if (info->pltreloc) {
    /* bytes per element */
    int bpe = d_plttype == DT_REL ? sizeof(Elf32_Rel) : sizeof(Elf32_Rela);
...
void _ldmapsect(int fd, unsigned long baseaddr, Elf32_Phdr *phdr, int textrel) {
  ...
  if (map_len != 0) {
    if (mmap((char *)map_addr, map_len, perms,
             ((perms & PROT_WRITE) ? MAP_PRIVATE : MAP_SHARED) | MAP_FIXED, fd,
             file_addr) == MAP_FAILED) {
  ...
  if (map_len != 0) {
    ...
    int zfd = _ldzero();
    addr =
        mmap((char *)map_addr, map_len, perms, MAP_PRIVATE | MAP_FIXED, zfd, 0);
    if (addr == MAP_FAILED) {
      printf(err_zeromap);
      exit(1);
    }
...
void _ldloadobj(module_t *module) {
  ...
  /* compute image size */
  hdr =
      (Elf32_Ehdr *)mmap(0, pagesize, PROT_READ | PROT_EXEC, MAP_SHARED, fd, 0);
