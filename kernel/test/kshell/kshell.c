 ...
 */
static int kshell_find_redirection(kshell_t *ksh, char *line, char *redirect_in,
                                   char *redirect_out, int *append) {
  int retval;
 ...
 * otherwise
 */
static int kshell_find_next_arg(kshell_t *ksh, char *line, char **arg,
                                size_t *arglen)
 ...
 */
static void kshell_get_args(kshell_t *ksh, char *buf, char **argv, int max_args,
                            int *argc) {
  size_t arglen;
...
int kshell_redirect(kshell_t *ksh, const char *redirect_in,
  ...
  if (redirect_out[0] != '\0') {
    int flags = append ? O_WRONLY | O_CREAT | O_APPEND : O_WRONLY | O_CREAT;
