...
#define LOCAL_APIC_SPUR_ADDR                                                   \
  (*(volatile uint32)t *)(apic->at_addr + LOCAL_APIC_SPURIOUS)
...
static uint32_t __lapic_getid(void) { return (LAPICID >> 24) & 0x0f; }

static uint32_t __lapic_getver(void) { return LAPICVER & 0xff; }
...
static void ioapic_write(uintptr_t ioapic_addr, uint8_t reg_offset,
                         uint32_t value) {
...
static void __ioapic_setredir(uint32_t irq, uint8_t intr) {
  ...
  uint32_t data =
      ioapic_read((uintptr_t)ioapic->at_addr, IRQ_TO_OFFSET(irq, 0));
...
static void apic_enable() {
  ...
  *(uint32_t *)(apic->at_addr + LOCAL_APIC_LDR) =
...
void apic_enable_periodic_timer(uint32_t freq) {
  ...
  *(uint32_t *)(apic->at_addr + LOCAL_APIC_LVT_TMR) = LOCAL_APIC_DISABLE;
  /* some math */
  cpubusfreq =
      ((0xffffffff - *(uint32_t *)(apic->at_addr + LOCAL_APIC_TMRINITCNT)) +
       1) *
      16 * 100;
  ...
  *(uint32_t *)(apic->at_addr + LOCAL_APIC_LVT_TMR) =
      32 | LOCAL_APIC_TMR_PERIODIC;
  *(uint32_t *)(apic->at_addr + LOCAL_APIC_TMRDIV) = 0x03;
}
...
uint8_t apic_getipl() { return LAPICTPR & 0xff; }

void apic_setipl(uint8_t ipl) { LAPICTPR = ipl; }
