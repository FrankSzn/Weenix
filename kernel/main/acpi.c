...
static void *__rsdp_search() {
  ...
  if (ebda >= (void *)EBDA_MIN && ebda < (void *)EBDA_MAX &&
      0 == ((uint32_t)ebda) % RSDP_ALIGN) {
    /* if EBDA pointer is valid search there first */
    uint8_t *p;
    for (p = ebda; (uint8_t *)p <= (uint8_t *)EBDA_MAX - sizeof(*rsdp);
         p += RSDP_ALIGN) {
      if (0 == memcmp(p, rsdp_sig, sizeof(rsdp_sig)) &&
          0 == __acpi_checksum(p, sizeof(struct rsdp))) {
        rsdp = (struct rsdp *)p;
        break;
      }
    }
  ...
  if (NULL == rsdp) {
    ...
    uint8_t *p;
    for (p = (uint8_t *)0xe0000;
         (uint8_t *)p <= (uint8_t *)0xfffff - sizeof(*rsdp); p += RSDP_ALIGN) {
      if (0 == memcmp(p, rsdp_sig, sizeof(rsdp_sig)) &&
          0 == __acpi_checksum(p, sizeof(struct rsdp))) {
        rsdp = (struct rsdp *)p;
        break;
...
static void *_acpi_load_table(uintptr_t paddr) {
  struct acpi_header *tmp =
      (struct acpi_header *)(pt_phys_tmp_map(
                                 (uintptr_t)PAGE_ALIGN_DOWN(paddr)) +
                             (PAGE_OFFSET(paddr)));
  ...
  while (size > 0) {
    source = (uint8_t *)pt_phys_tmp_map((uintptr_t)PAGE_ALIGN_DOWN(paddr)) +
             (PAGE_OFFSET(paddr));
...
void acpi_init() {
   ...
   * memory, therefore we must use the phys_tmp_map functionallity of page
   * tables */
  ...
  KASSERT(
      0 == __acpi_checksum((void *)rsd_table, rsd_table->rt_header.ah_size) &&
