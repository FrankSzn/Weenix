...
static int _elf32_map_segment(vmmap_t *map, vnode_t *file, int32_t memoff,
  ...
  if (0 < filesz) {
    ...
    if (!vmmap_is_range_empty(map, lopage, npages)) {
      ...
      return -ENOEXEC;
               ...
               (ret = vmmap_map(map, file, lopage, npages, perms,
  ...
  if (memsz > filesz) {
    ...
    if (npages > 1 && !vmmap_is_range_empty(map, lopage + 1, npages - 1)) {
      ...
      if (NULL == (buf = page_alloc()))
        return -ENOMEM;
      if (!(0 > (ret = file->vn_ops->read(file,
                                          (off_t)PAGE_ALIGN_DOWN(off + filesz),
                                          buf, PAGE_OFFSET(addr + filesz))))) {
        ret = vmmap_write(map, PAGE_ALIGN_DOWN(addr + filesz), buf,
...
static int _elf32_load_ehdr(int fd, Elf32_Ehdr *header, int interp) {
  ...
  if (0 > (err = do_read(fd, header, sizeof(*header)))) {
    return err;
             ...
             0 != memcmp(&header->e_ident[0], ELFMAG, SELFMAG)) {
...
static int _elf32_load_phtable(int fd, Elf32_Ehdr *header, char *pht,
                               size_t size) {
  int err = 0;
...
static int _elf32_map_progsegs(vnode_t *vnode, vmmap_t *map, Elf32_Ehdr *header,
                               char *pht, int32_t memoff) {
  int err = 0;
 ...
 * there
 * is no interpreter) or -errno on error. If there is no interpreter section
 * then
 * phinterp is set to NULL. If there is more than one interpreter then -EINVAL
 * is
 * returned. */
static int _elf32_find_phinterp(Elf32_Ehdr *header, char *pht,
                                Elf32_Phdr **phinterp) {
  int err = 0;
  *phinterp = NULL;
 ...
 */
static void _elf32_calc_progbounds(Elf32_Ehdr *header, char *pht, void **low,
                                   void **high) {
...
static size_t _elf32_calc_argsize(char *const argv[], char *const envp[],
                                  Elf32_auxv_t *auxv, size_t phtsize, int *argc,
                                  int *envc, int *auxc) {
  size_t size = 0;
  int i;
  /* All strings in argv */
 ...
 */
static void _elf32_load_args(vmmap_t *map, void *arglow, size_t argsize,
                             char *buf, char *const argv[], char *const envp[],
                             Elf32_auxv_t *auxv, int argc, int envc, int auxc,
                             int phtsize) {
  int i;
  ...
  *((int *)buf) = argc;
  ...
  size_t veclen = (argc + 1 + envc + 1) * sizeof(char *) +
                  (auxc + 1) * sizeof(Elf32_auxv_t);

  char *vecstart =
      buf + sizeof(int) +
      3 * sizeof(void *); /* Beginning of argv (in kernel buffer) */

  char *vvecstart = ((char *)arglow) + sizeof(int) +
                    3 * sizeof(void *); /* Beginning of argv (in user space) */

  char *strstart =
      vecstart +
      veclen; /* Beginning of first string pointed to by argv
                                                     (in kernel buffer) */
...
static int _elf32_load(const char *filename, int fd, char *const argv[],
  ...
  if (NULL != phinterp) {
    ...
    /* Read in the program header table */
    if (0 > (err = _elf32_load_phtable(interpfd, &interpheader, interppht,
                                       interpphtsize))) {
      goto done;
    }
    ...
    if (0 > (err = _elf32_find_phinterp(&interpheader, interppht,
                                        &interpphinterp))) {
      goto done;
    }
    ...
    uint32_t interppagebase =
        (uint32_t)vmmap_find_range(map, interpnpages, VMMAP_DIR_HILO);
    ...
    int32_t interpoff =
        (int32_t)interppagebase - (int32_t)ADDR_TO_PN(interplow);
    ...
    if (0 > (err = _elf32_map_progsegs(interpfile->f_vnode, map, &interpheader,
                                       interppht, interpoff))) {
      goto done;
    }
   ...
   * stack is located). I suppose we can add this "extra page for magic data"
   * too */
  uint32_t stack_lopage =
      ADDR_TO_PN(proglow) - (DEFAULT_STACK_SIZE / PAGE_SIZE) - 1;
  ...
  int argc, envc, auxc;
  size_t argsize =
      _elf32_calc_argsize(argv, envp, auxv, phtsize, &argc, &envc, &auxc);
  ...
  _elf32_load_args(map, arglow, argsize, argbuf, argv, envp, auxv, argc, envc,
                   auxc, phtsize);

  dbg(DBG_ELF, "Past the point of no return. Swapping to map at 0x%p, setting "
               "brk to 0x%p\n",
      map, proghigh);
  ...
  *eip = (uint32_t)entry;
  *esp = ((uint32_t)arglow) -
         4; /* Space on the user stack for the (garbage) return address */
