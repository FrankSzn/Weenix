...
static void *shadowd(int arg1, void *arg2) {
  while (1) {
    proc_t *p;
    /* for each process, go through its vmareas */
    list_iterate_begin(proc_list(), p, proc_t, p_list_link) {
      /* all of the dead process's shadow objects will be takenen care of by
       * init */
      if (PROC_RUNNING == p->p_state) {
        ...
        list_iterate_begin(&p->p_vmmap->vmm_list, vma, vmarea_t, vma_plink) {
          ...
          while (NULL != o && NULL != o->mmo_shadowed) {
            ...
            /* iff the object has only one parent, and is not right under
             * vm_area */
            KASSERT(o != last);
            if (o->mmo_refcount - o->mmo_nrespages == 1) {
              /* migrate all its pages to last, and remove it from the shadow
               * tree */
              pframe_t *pf;
...
static __attribute__((unused)) void shadowd_init() {
