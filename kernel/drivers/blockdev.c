...
static int blockdev_lookuppage(mmobj_t *o, uint32_t pagenum, int forwrite,
                               pframe_t **pf);
...
static mmobj_ops_t blockdev_mmobj_ops = {.ref = blockdev_ref,
                                         .put = blockdev_put,
...
int blockdev_register(blockdev_t *dev) {
  ...
  if (!dev || (NULL_DEVID == dev->bd_id) || !(dev->bd_ops))
    return -1;
...
clean:
  list_iterate_begin(&dev->bd_mmobj.mmo_respages, pf, pframe_t, pf_olink) {
