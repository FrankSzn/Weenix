...
} ATA_CHANNELS[2] = {{ATA_PRIMARY_CMD_BASE, ATA_PRIMARY_CTRL_BASE,
                      INTR_DISK_PRIMARY, NULL, NULL, NULL},
                     {ATA_SECONDARY_CMD_BASE, ATA_SECONDARY_CTRL_BASE,
                      INTR_DISK_SECONDARY, NULL, NULL, NULL}};
...
#define ATA_REG_DRIVEHEAD                                                      \
  0x06                       /* Special drive info (used to set master/slave) */
 ...
 * status does) */
static void ata_sync(uint8_t channel) { ata_inb_altstatus(channel); }

/* Sync and wait a bit */
static void ata_pause(uint8_t channel) {
  ata_sync(channel);
  ndelay(400);
}
...
static int ata_write(blockdev_t *bdev, const char *data, blocknum_t blocknum,
                     unsigned int count);
static int ata_do_operation(ata_disk_t *adisk, char *data, blocknum_t sectornum,
                            int write);
static void ata_intr(regs_t *regs, void *arg);

static blockdev_ops_t ata_disk_ops = {.read_block = ata_read,
...
void ata_init() {
  ...
  for (ii = 0; ii < NDISKS; ii++) {
    ...
    /* Now the drive is no longer busy, poll until the error bit is set or drq
     * is set */
    while (1) {
      status = ata_inb_reg(channel, ATA_REG_STATUS);
    ...
    if (NULL == (adisk = (ata_disk_t *)kmalloc(sizeof(ata_disk_t))))
    ...
    intr_register(ATA_CHANNELS[adisk->ata_channel].atac_intr, ata_intr_wrapper);
    ATA_CHANNELS[adisk->ata_channel].atac_intr_handler = ata_intr;
    ATA_CHANNELS[adisk->ata_channel].atac_intr_arg = adisk;
    ATA_CHANNELS[adisk->ata_channel].atac_busmaster =
        ata_setup_busmaster(adisk);
    ...
    if (ATA_CHANNELS[i].atac_intr == regs->r_intr) {
      if (NULL == ATA_CHANNELS[i].atac_intr_handler)
        panic("No handler registered "
              "for ATA channel %d!\n",
              i);
      ATA_CHANNELS[i].atac_intr_handler(regs, ATA_CHANNELS[i].atac_intr_arg);
...
uint16_t ata_setup_busmaster(ata_disk_t *adisk) {
  ...
  uint32_t busmaster_base =
      ide->pci_bar[4].base_addr + (adisk->ata_channel * 8);

  if (busmaster_base == 0) {
    panic("No valid busmastering address\n");
  }

  KASSERT(busmaster_base != 0 &&
          "Disk device should not have 0 for the busmaster register");
