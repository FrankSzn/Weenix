 ...
 */
static uint32_t pci_config_read(uint16_t bus, uint8_t device, uint8_t func,
                                uint8_t reg_off, uint8_t length) {
  uint8_t reg = reg_off & 0xFC;
  uint8_t offset = reg_off % 0x04;

  outl(PCI_CONFIGURATION_ADDRESS,
       0x80000000 | (bus << 16) | (device << 11) | (func << 8) | reg);
 ...
 */
static void pci_config_write_byte(uint8_t bus, uint8_t device, uint8_t func,
                                  uint8_t reg, uint8_t val) {
  outl(PCI_CONFIGURATION_ADDRESS,
       0x80000000 | (bus << 16) | (device << 11) | (func << 8) | (reg & 0xfc));
 ...
 * Basically used to configure a device, in practice right now only the dick
 */
static void pci_config_write_short(uint8_t bus, uint8_t device, uint8_t func,
 ...
 * Basically used to configure a device, in practice right now only the dick
 */
static void pci_config_write_word(uint8_t bus, uint8_t device, uint8_t func,
  ...
  for (bus = 0; bus < PCIBUSES; ++bus) {
    for (device = 0; device < PCIDEVICES; ++device) {
      ...
      for (func = 0; func < funcCount; ++func) {
        uint16_t vendorId =
            pci_config_read(bus, device, func, PCI_VENDOR_ID, 2);
        if (vendorId && vendorId != 0xFFFF) {
          ...
          dev->pci_deviceid =
              pci_config_read(bus, device, func, PCI_DEVICE_ID, 2);
          dev->pci_classid = pci_config_read(bus, device, func, PCI_CLASS, 1);
          dev->pci_subclassid =
              pci_config_read(bus, device, func, PCI_SUBCLASS, 1);
          dev->pci_interfaceid =
              pci_config_read(bus, device, func, PCI_INTERFACE, 1);
          dev->pci_revid = pci_config_read(bus, device, func, PCI_REVISION, 1);
          dev->pci_irq = pci_config_read(bus, device, func, PCI_IRQLINE, 1);
          dbg(DBG_DISK, "DevID: %x, Class: %x, Subclass: %x, Interface: %x, "
                        "IRQ Line: %x\n",
              dev->pci_deviceid, dev->pci_classid, dev->pci_subclassid,
              dev->pci_interfaceid, dev->pci_irq);
          /* Read BAR data */
          uint8_t i = 0;
          for (i = 0; i < 6; i++) {
            if (i < 2 || !(headerType & 0x01)) {
              dev->pci_bar[i].base_addr =
                  pci_config_read(bus, device, func, PCI_BAR0 + i * 4, 4);
              if (dev->pci_bar[i].base_addr) {
                ...
                dev->pci_bar[i].mem_size =
                    (~(pci_config_read(bus, device, func, PCI_BAR0 + i * 4,
                                       4)) |
                     0x0f) +
                    1;
  ...
  list_iterate_begin(&pci_list, dev, pcidev_t, pci_link) {
    ...
    if (((class == PCI_LOOKUP_WILDCARD) || (dev->pci_classid == class)) &&
        ((subclass == PCI_LOOKUP_WILDCARD) ||
         (dev->pci_subclassid == subclass)) &&
        ((interface == PCI_LOOKUP_WILDCARD) ||
         (dev->pci_interfaceid == interface))) {
      return dev;
    }
  }
  list_iterate_end();
...
uint32_t pci_read_config(pcidev_t *dev, uint8_t reg_off, uint8_t length) {
  return pci_config_read(dev->pci_bus, dev->pci_device, dev->pci_func, reg_off,
                         length);
}
 ...
 */
void pci_write_config(pcidev_t *dev, uint8_t reg_off, uint32_t val,
                      uint8_t length) {
  /* only allow writing of a byte, a word, or a short */
  KASSERT(length == 1 || length == 4 || length == 2);
  switch (length) {
  case 1:
    pci_config_write_byte(dev->pci_bus, dev->pci_device, dev->pci_func, reg_off,
                          (uint8_t)val);
    break;
  case 2:
    KASSERT(val == (uint16_t)val);
    pci_config_write_short(dev->pci_bus, dev->pci_device, dev->pci_func,
                           reg_off, (uint16_t)val);
    break;
  case 4:
    pci_config_write_word(dev->pci_bus, dev->pci_device, dev->pci_func, reg_off,
                          val);
    break;
  default:
