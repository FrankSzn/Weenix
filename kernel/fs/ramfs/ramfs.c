 ...
 *
 *    o There is no support for fillpage, etc. (since we don't have VM yet
 *anyway)
 *
...
static int ramfs_write(vnode_t *file, off_t offset, const void *buf,
                       size_t count);
/* getpage */
...
static int ramfs_lookup(vnode_t *dir, const char *name, size_t name_len,
                        vnode_t **result);
static int ramfs_link(vnode_t *oldvnode, vnode_t *dir, const char *name,
                      size_t name_len);
...
/* Helper functions */
static int ramfs_alloc_inode(fs_t *fs, int type, devid_t devid) {
...
static int ramfs_lookup(vnode_t *dir, const char *name, size_t namelen,
                        vnode_t **result) {
  off_t i;
...
static int ramfs_unlink(vnode_t *dir, const char *name, size_t namelen) {
  vnode_t *vn;
  int ret;
  off_t i;
  ramfs_dirent_t *entry;
...
static int ramfs_readdir(vnode_t *dir, off_t offset, struct dirent *d) {
  ...
  while ((offset < (off_t)(RAMFS_MAX_DIRENT * sizeof(ramfs_dirent_t))) &&
