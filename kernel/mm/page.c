...
static struct pagegroup *_pagegroup_from_address(uintptr_t addr) {
  struct pagegroup *group;
 ...
 */
static inline uintptr_t _pagegroup_calculate_index(struct pagegroup *group,
                                                   uint32_t order,
                                                   uintptr_t addr) {
...
static void __page_split(struct pagegroup *group, uint32_t order) {
  KASSERT(0 < order);
  KASSERT(PAGE_NSIZES > order);
  KASSERT(!list_empty(&group->pg_freelist[order]));
  KASSERT(PAGE_SIZE >= sizeof(uintptr_t));

  uintptr_t target = (uintptr_t)list_head(&group->pg_freelist[order],
                                          struct freepage, fp_link);
  ...
  KASSERT(!bit_check(group->pg_map[order],
                     _pagegroup_calculate_index(group, order, target)));

  uintptr_t buddy = (target + ((1 << (order - 1)) << PAGE_SHIFT));
  list_insert_head(&group->pg_freelist[order - 1],
                   &((struct freepage *)target)->fp_link);
  list_insert_head(&group->pg_freelist[order - 1],
                   &((struct freepage *)buddy)->fp_link);
 ...
 */
static struct pagegroup *_page_split(int order) {
#ifdef __SHADOWD__
 ...
 * @return the address of the free memory or null if no memory could be
 *allocated
 */
static void *_page_alloc_order(uint32_t order) {
  uintptr_t addr;
  struct pagegroup *group;
...
found:
  addr = (uintptr_t)list_head(&group->pg_freelist[order], struct freepage,
                              fp_link);
  list_remove_head(&group->pg_freelist[order]);
  if (PAGE_NSIZES - 1 > order)
    bit_flip(group->pg_map[order + 1],
...
static void __page_join(struct pagegroup *group, int order, uintptr_t addr) {
  uintptr_t index;
  while (PAGE_NSIZES - 1 > order &&
         !bit_check(group->pg_map[order + 1],
                    index = _pagegroup_calculate_index(group, order + 1,
                                                       (uintptr_t)addr))) {
    uintptr_t offset = addr - group->pg_baseaddr;
    uintptr_t buddy = addr +
                      ((1 << order) << PAGE_SHIFT) *
                          ((((offset >> PAGE_SHIFT) >> order) & 0x1) ? -1 : 1);

    KASSERT(0 == (offset & ((1 << order) - 1)));

    dbg(DBG_PAGEALLOC, "joining 0x%.8x and 0x%.8x (%u) into 0x%.8x\n", addr,
        buddy, order, MIN(offset, buddy));
    ...
    ++order;
    list_insert_head(&group->pg_freelist[order],
                     &((struct freepage *)addr)->fp_link);

    if (PAGE_NSIZES - 1 > order)
      bit_flip(group->pg_map[order + 1],
               _pagegroup_calculate_index(group, order + 1, (uintptr_t)addr));
  }
}
 ...
 */
static void _page_free_order(void *addr, int order) {
#ifdef MM_POISON
  /*
  ...
  dbg(DBG_MM,
      "page_free: freed %d pages (addr 0x%p); %u pages currently free\n",
