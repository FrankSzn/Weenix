...
#define rear_rz(cache, obj)                                                    \
  (*(uintptr_t *)(((uintptr_t)(obj)) + (cache)->sa_objsize - sizeof(uintptr_t)))
...
static size_t _slab_size(size_t objsize, size_t nobjs) {
  return (nobjs * (objsize + sizeof(struct slab_bufctl)) + sizeof(struct slab));
}

static int _slab_nobjs(size_t objsize, size_t order) {
  return (((PAGE_SIZE << order) - sizeof(struct slab)) /
          (objsize + sizeof(struct slab_bufctl)));
}

static int _slab_waste(size_t objsize, int order) {
...
static void _calc_slab_size(struct slab_allocator *allocator) {
  int best_order;
  int best_waste;
  int order;
  int minorder;
...
static void _allocator_init(struct slab_allocator *allocator, const char *name,
                            size_t size) {
#ifdef SLAB_REDZONE
  /*
...
struct slab_allocator *slab_allocator_create(const char *name, size_t size) {
  struct slab_allocator *allocator;

  allocator =
      (struct slab_allocator *)slab_obj_alloc(&slab_allocator_allocator);
  if (!allocator)
    return NULL;
...
static int _slab_allocator_grow(struct slab_allocator *allocator) {
  void *addr;
  void *obj;
  int ii, npages;
  struct slab *slab;
...
/* The last bufctl is the tail of the list. */
  ...
  dbg(DBG_MM, "Growing cache \"%s\" (0x%p), new slab 0x%p "
              "(%d pages)\n",
      allocator->sa_name, allocator, slab, 1 << allocator->sa_order);
...
void *slab_obj_alloc(struct slab_allocator *allocator) {
  struct slab *slab;
  void *obj;
...
void slab_obj_free(struct slab_allocator *allocator, void *obj) {
  struct slab *slab;
...
void *kmalloc(size_t size) {
  ...
  for (order = KMALLOC_SIZE_MIN_ORDER; order <= KMALLOC_SIZE_MAX_ORDER;
       order++, cs++) {
...
void slab_init() {
  ...
  for (order = KMALLOC_SIZE_MIN_ORDER; order <= KMALLOC_SIZE_MAX_ORDER;
       order++, cs++) {
    if (NULL == (*cs = slab_allocator_create(
