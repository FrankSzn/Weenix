...
static char *type_strings[] = {"ERROR: type = 0",  "Usable",   "Reserved",
                               "ACPI Reclaimable", "ACPI NVS", "GRUB bad ram"};

static size_t type_count = sizeof(type_strings) / sizeof(char *);

#define NEXT_MMAP(m)                                                           \
  ((multiboot_memory_map_t *)(((uintptr_t)(m)) + (m)->size + sizeof((m)->size)))
uintptr_t phys_detect_highmem(void) {
  KASSERT(boot_info->flags & MULTIBOOT_INFO_MEM_MAP &&
          "No Memory mapping information provided");
  void *last_mmap =
      (void *)((uintptr_t)boot_info->mmap_addr + boot_info->mmap_length);
  dbgq(DBG_MM,
       "Physical Memory Info: KERNEL_PHYS_BASE: 0x%.8x, %d bytes of info\n",
  ...
  for (multiboot_memory_map_t *mmap =
    ...
    if (1 /* Usable */ == type && KERNEL_PHYS_BASE >= base &&
        KERNEL_PHYS_BASE < base + len) {
      if (ret == 0) {
